...
* DFS의 일종으로 스택을 이용해 구현.


* 전위 순회(Preorder Traversal) : 현재 노드 -> 왼쪽 서브 트리 -> 오른쪽 서브 트리
* 중위 순회(Inorder Traversal) : 왼쪽 서브 트리 -> 현재 노드 -> 오른쪽 서브 트리
* 후위 순회(Postorder Traversal) : 왼쪽 서브 트리 -> 오른쪽 서브 트리 -> 현재 노드ㄴ
